...
class Vertex<T> implements VertexInterface<T>
{
    ...
    public void deleteNeighbor(int position) {
        edgeList.remove(position);
    } // end deleteNeighbor

    public int getNeighborPosition(T neighbor) {
        Iterator<VertexInterface<T>> iter = this.getNeighborIterator();
        int position = 0;
        while (iter.hasNext()) {
            if(!neighbor.equals(iter.next())) {
                position++;
            }
            else {
                break;
